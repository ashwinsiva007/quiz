        ...
        {state === 'QUESTION_ACTIVE' && currentQuestion && (
          <div className="space-y-4">
            ...
            {participant?.hasAnswered ? (
              <div className="bg-[#151c2e] border border-emerald-500/40 rounded-2xl p-8 text-center shadow-2xl space-y-4 animate-scale-in">
                ...
                <div>
                  <h4 className="text-xl font-bold text-white">Answer Submitted!</h4>
                  <p className="text-xs text-slate-400 mt-1">Waiting for other participants & results…</p>
                </div>
              </div>
            ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                {currentQuestion.options.map((opt, idx) => (
                  <button
                    ...
                    className={`bg-gradient-to-r ${getOptionStyle(idx)} border text-white p-5 rounded-2xl font-bold text-left shadow-lg active:scale-98 transition-transform flex items-center gap-3`}
                  >
        ...
        {state === 'QUESTION_RESULTS' && studentResult && (
          <div className="space-y-4">
            ...
            {studentResult.isCorrect ? (
              <div className="bg-gradient-to-b from-emerald-950/80 to-[#151c2e] border-2 border-emerald-500 rounded-3xl p-6 text-center shadow-2xl space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-amber-400 font-mono mt-1">
                    +{studentResult.lastQuestionScore} PTS EARNED
                  </p>
                </div>
                <div className="text-xs text-slate-300 pt-2 border-t border-slate-800">
                  Total Score: <strong className="text-white font-mono">{studentResult.score} PTS</strong>
                </div>
              </div>
            ) : studentResult.selectedOption !== null && studentResult.selectedOption !== undefined ? (
              <div className="bg-gradient-to-b from-rose-950/80 to-[#151c2e] border-2 border-rose-500 rounded-3xl p-6 text-center shadow-2xl space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Correct Answer was Option <strong className="text-emerald-400">{getOptionLetter(studentResult.correctOption)}</strong>
                  </p>
                </div>
                <div className="text-xs text-slate-300 pt-2 border-t border-slate-800">
                  Total Score: <strong className="text-white font-mono">{studentResult.score} PTS</strong>
